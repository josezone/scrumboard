...
class SpreadSheet {
  ...
  protected alphabets: Array<string> = [
    "A",
    "B",
    "C",
    "D",
  ...
  protected getBugs(bugs: Array<any>) {
    ...
    bugs?.map((bug, i: number) => {
      bugText +=
        ...
        "/" +
        (new Date(bug.date).getMonth() + 1).toString().padStart(2, "0") +
        " - " +
        bug.bug;
      if (i + 1 !== bugs.length) {
        bugText += "\n";
  ...
  mergeCells(type: string) {
    ...
    this.worksheet
      .getColumn(type)
      .eachCell(function (cell: any, rowNumber: any, i: any) {
        mergeCells.push({
    ...
    Object.keys(groupCells).map((key, i) => {
      if (i !== 0) {
        ...
        if (project.length > 0) {
          ...
          this.worksheet.getCell(startAddress).alignment = {
            horizontal: "center",
            vertical: "middle",
          };
          if (type === "project") {
